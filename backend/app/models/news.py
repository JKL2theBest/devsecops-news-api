...
class News(Base):
    ...
    published_at: Mapped[datetime.datetime] = mapped_column(DateTime(timezone=True), default=func.now())
    cover_image_url: Mapped[str | None] = mapped_column(String(255))
    author_id: Mapped[uuid.UUID] = mapped_column(pgUUID(as_uuid=True), ForeignKey("users.id"))
